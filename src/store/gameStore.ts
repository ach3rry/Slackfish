...
const randomTrashTalk = () =>
  BOSS_TRASH_TALK[Math.floor(Math.random() * BOSS_TRASH_TALK.length)] ?? BOSS_TRASH_TALK[0]

const initialState = () => ({
  phase: 'start' as GamePhase,
  salary: GAME_CONFIG.economy.initialSalary,
  fish: 0,
  elapsedSeconds: 0,
  currentArea: 'workstation' as AreaId,
  currentAction: 'idle' as PlayerAction,
  bossStatus: 'resting' as BossStatus,
  threatText: '老板休息中',
  cooldownUntil: 0,
  stunnedUntil: 0,
  catchNotice: null,
  salaryFlash: null as FlashTone,
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  setAction: (action, force = false) => {
    ...
    if (!force) {
      ...
      if (timestamp < state.cooldownUntil || timestamp < state.stunnedUntil) {
        return false
      }
    ...
    set({
      ...
      cooldownUntil:
        force || action === 'moving'
          ? state.cooldownUntil
          : timestamp + GAME_CONFIG.timing.actionCooldownMs,
    })
    return true
  },
  ...
  tickEconomy: (deltaSeconds) => {
    ...
    if (state.currentAction === 'moving' || now() < state.stunnedUntil) {
      set((current) => ({ elapsedSeconds: current.elapsedSeconds + deltaSeconds }))
      return { fishGain: 0, salaryGain: 0 }
    }
    ...
    set((current) => {
      ...
      if (nextFish >= GAME_CONFIG.economy.targetFish && nextSalary > 0) {
        ...
        const entry: LeaderboardEntry = {
          ...
          elapsedSeconds,
          salary: Math.round(nextSalary),
          fish: Math.round(nextFish),
          title,
        }
        return {
          salary: nextSalary,
          ...
          phase: 'won',
          finalTitle: title,
          salaryFlash: salaryGain > 0 ? 'gain' : current.salaryFlash,
          leaderboard: saveLeaderboardEntry(entry),
        }
      }
  ...
  applyCatch: (amount, title) => {
    ...
    set((state) => {
      ...
      return {
        salary: nextSalary,
        phase: nextSalary <= 0 ? 'lost' : state.phase,
        ...
        salaryFlash: 'loss',
        threatText: nextSalary <= 0 ? '工资清零' : '被老板抓包',
      }
    })
  ...
  markGuideSeen: () => {
    window.localStorage.setItem(GAME_CONFIG.timing.guideStorageKey, '1')
    set({ guideOpen: false })
  },
